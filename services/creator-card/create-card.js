...
const { generateSlug, isValidSlug, appendRandomSuffix } = require('./generate-slug');
...
async function createCard(serviceData, options = {}) {
  ...
  try {
    ...
    if (data.links && data.links.length > 50) {
      throwAppError(CreatorCardMessages.TOO_MANY_LINKS, ERROR_CODE.INVLDDATA);
    }
    ...
    if (data.service_rates && data.service_rates.rates && data.service_rates.rates.length > 20) {
      throwAppError(CreatorCardMessages.TOO_MANY_RATES, ERROR_CODE.INVLDDATA);
    }
